...
if __name__ == '__main__':
    # Choosing axiom set
    fileName = "CAT001-0"
    axioms = parse_tptp_clauses(f'{constants.CLAUSIFIED_AXIOMS}/{fileName}.ax_claused.txt')

    for k in range(10):
        N=50
        T=8.0

        problems = generate_problem(axioms, N, T)

        write_to_tptp(problems, f'{constants.GENERATED_PROBLEM}/gen_prob_{fileName}_N={N}_T={T}_{k}.p')
